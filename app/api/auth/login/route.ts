...
export async function POST(req: Request) {
  ...
  // IP 기반 rate limit (15분에 5회)
  const ipRl = rateLimit({ key: `login:ip:${ip}`, windowMs: 15 * 60 * 1000, max: 5 })
  if (!ipRl.ok) {
  ...
  // 사용자명 기반 추가 rate limit (15분에 5회)
  const userRl = rateLimit({ key: `login:user:${id}`, windowMs: 15 * 60 * 1000, max: 5 })
  if (!userRl.ok) {
    return NextResponse.json(
