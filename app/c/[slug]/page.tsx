...
export default function EventContractUploadPage() {
  ...
  async function handleVerify() {
    ...
    try {
      ...
      if (!res.ok) {
        setVerifyError(data.error || '일치하는 계약서를 찾을 수 없습니다.');
        return;
      }
    ...
    <div className="min-h-screen bg-[var(--brand-lime)]">
      ...
      <header className="border-b border-[var(--brand-dark)]">
        <div className="max-w-7xl mx-auto px-6 py-5 flex justify-between items-center">
          ...
          <div className="text-xs uppercase tracking-[0.15em]">계약서 업로드</div>
        </div>
      </header>

      <div className="max-w-lg mx-auto px-4 py-10">
        {/* 행사 정보 (고정) */}
        <div className="border-2 border-[var(--brand-dark)] bg-white p-5 mb-6">
          <div className="text-xs uppercase tracking-[0.15em] opacity-50 mb-1">행사</div>
          <div className="font-serif text-2xl break-keep">{event.title}</div>
          <div className="text-sm opacity-60 mt-1">{event.venue} · {event.dates[0]} ~ {event.dates[event.dates.length - 1]}</div>
        </div>
              ...
              <div className="bg-white border-2 border-[var(--brand-dark)] p-6">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    ...
                    <div className="text-sm opacity-60">{event.title}</div>
                  </div>
                </div>
                ...
                <div className="border border-gray-300 p-4 bg-gray-50">
                  <label className="flex items-start gap-3 cursor-pointer">
                    <input
                      ...
                      className="mt-0.5 accent-[var(--brand-dark)]"
                    />
              ...
              <div className="border-2 border-[var(--brand-dark)]">
                ...
                <div className="border-t border-[var(--brand-dark)]/20 p-4">
                  ...
                  {verifyResult.mimeType.startsWith('image/') ? (
                    <img src={verifyResult.fileUrl} alt="업로드된 계약서" className="w-full border border-gray-200" />
                  ) : verifyResult.mimeType === 'application/pdf' ? (
                    <iframe src={verifyResult.fileUrl} title="업로드된 계약서" className="w-full border border-gray-200" style={{ height: '480px' }} />
                  ) : null}
                  <a
